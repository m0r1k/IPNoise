...
static ngx_int_t ngx_parse_hostos_url(ngx_pool_t *pool, ngx_url_t *u);
...
ngx_parse_url(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    if (ngx_strncasecmp(p, (u_char *) "hostos:", 7) == 0) {
        return ngx_parse_hostos_url(pool, u);
    }
...
static ngx_int_t
ngx_parse_hostos_url(ngx_pool_t *pool, ngx_url_t *u)
{
    u_char              *p          = NULL;
    u_char              *proto      = NULL;
    u_char              *host       = NULL;
    ...
    sin->sin_family = AF_HOSTOS;
    u->family       = AF_HOSTOS;

    enum {
        HOSTOS_URL_STATE_PROTO,
        HOSTOS_URL_STATE_HOST,
        HOSTOS_URL_STATE_PORT
    } state;

    // parse
    state = HOSTOS_URL_STATE_PROTO;
    for (i = 0; i < u->url.len; i++){
        u_char cur_char = u->url.data[i];

        switch (state){
            case HOSTOS_URL_STATE_PROTO:
                if (':' == cur_char){
                    state = HOSTOS_URL_STATE_HOST;
                } else {
                    if (!proto){
                        proto = &u->url.data[i];
                    }
            ...
            case HOSTOS_URL_STATE_HOST:
                if (':' == cur_char){
                    state = HOSTOS_URL_STATE_PORT;
                } else {
                    if (!host){
                        host = &u->url.data[i];
                    }
    ...
    // setup listen addr, hardcode here
    // we will enable only loopback for hostos listen
    // because of security reasons
    u->host.len  = host_len;
    u->host.data = host;

    sin->sin_addr.s_addr = ngx_inet_addr(
        u->host.data,
        u->host.len
    );
    ...
    if (INADDR_NONE != sin->sin_addr.s_addr){
        ...
        u->addrs[0].name.len = ngx_sprintf(p,
            "%V:%d",
            &u->host,
            u->port
        ) - p;
