...
ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (tries = 5; tries; tries--) {
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ...
            // FYI socket protocol family is not same as bind address family
            // nginx do not known about it
            // workaround for hostos support
            if (AF_HOSTOS == ls[i].sockaddr->sa_family){
                ls[i].sockaddr->sa_family = AF_INET;
                res = bind(s, ls[i].sockaddr, ls[i].socklen);
                ls[i].sockaddr->sa_family = AF_HOSTOS;
            } else {
                res = bind(s, ls[i].sockaddr, ls[i].socklen);
            }
