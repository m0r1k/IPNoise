...
ClientHandlerTelnetCommandAddGroup::ClientHandlerTelnetCommandAddGroup(
...
const char *ClientHandlerTelnetCommandAddGroup::getName()
{
    return "api_addGroup";
};

int ClientHandlerTelnetCommandAddGroup::run(int argc, char* const* argv)
{
    PDEBUG(5, "addgroup command received\n");
    ...
    placeholders["name"]    =   "New group";
    placeholders["parent"]  =   "0";

    // init command prototype
    res = do_init(argv[0],
        "<ipnoise ver=\"0.01\" sessid=\"[[sessid]]\">\n"
        "    <commands>\n"
        "       <command type=\"addGroup\" ver=\"0.01\">\n"
        "           <group name=\"[[name]]\" parent=\"[[parent]]\"/>\n"
        "       </command>\n"
        "    </commands>\n"
        "</ipnoise>\n"
    );
    ...
    while(1) {
        ...
        switch (option) {
            ...
            case '1':
                placeholders["name"] = optarg;
                break;

            case '2':
                placeholders["parent"] = optarg;
                break;
    ...
    if (show_usage){
        snprintf(buffer, sizeof(buffer),
            ...
            "             --parent          new group's parent group ID     \n"
            "         -s, --send            Send command                    \n"
            "\n",
            argv[0]
        );
