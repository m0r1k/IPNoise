...
ClientHandlerTelnetCommandConferenceInvite::ClientHandlerTelnetCommandConferenceInvite(
...
const char *ClientHandlerTelnetCommandConferenceInvite::getName()
{
    return "api_conferenceInvite";
};

int ClientHandlerTelnetCommandConferenceInvite::run(int argc, char* const* argv)
{
    PDEBUG(5, "conferenceInvite command received\n");
    ...
    res = do_init(argv[0],
        ...
        "    <commands>\n"
        "       <command type=\"conferenceInvite\" ver=\"0.01\""
                    " conf_id=\"[[conf_id]]\""
                    " huid=\"[[huid]]\""
                    "/>\n"
        "    </commands>\n"
    ...
    if (show_usage){
        snprintf(buffer, sizeof(buffer),
            ...
            "             --conf_id         conference for invite           \n"
            "             --huid            huid for invite                 \n"
            "         -s, --send            Send command                    \n"
            "\n",
            argv[0]
        );
