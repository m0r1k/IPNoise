...
#include "clientHandlerTelnetCommandGetItem.h"

ClientHandlerTelnetCommandGetItem::ClientHandlerTelnetCommandGetItem(
...
const char *ClientHandlerTelnetCommandGetItem::getName()
{
    return "api_getItem";
};

int ClientHandlerTelnetCommandGetItem::run(int argc, char* const* argv)
{
    PDEBUG(5, "getiteminfo command received\n");
    ...
    res = do_init(argv[0],
        ...
        "    <commands>\n"
        "       <command type=\"getItem\" ver=\"0.01\" huid=\"[[huid]]\" />\n"
        "    </commands>\n"
        "</ipnoise>\n"
    );
    if (res){
    ...
    if (show_usage){
        snprintf(buffer, sizeof(buffer),
            ...
            "             --huid        item's huid (item for info)     \n"
            "         -s, --send        Send command                    \n"
            "\n",
            argv[0]
        );
