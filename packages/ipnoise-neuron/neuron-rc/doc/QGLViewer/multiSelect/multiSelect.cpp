...
void Viewer::draw()
{
    ...
    for (it = selection_.begin(), end = selection_.end();
        it != end;
        ++it)
    {
        objects_.at(*it)->draw();
...
{
    Vec averagePosition;
    ManipulatedFrameSetConstraint *mfsc = NULL;

    mfsc = (ManipulatedFrameSetConstraint*)(manipulatedFrame()->constraint());
    mfsc->clearSet();
