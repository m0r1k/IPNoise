...
static const struct pci_device_id ipnoise_pci_tbl[] = {
    ...
    const struct pci_device_id  *ent)
{
	...
	res = pci_set_dma_mask(pdev, IPNOISE_DMA_MASK);
    if (res){
    ...
    int             flags)
{
        ...
        &&  last_packet->data)
    {
        ...
        pdebug(20, "copy_to_iter '%d' byte(s),"
            " res: '%d'\n",
            last_packet->data_size,
            res
        );
    ...
    size_t          total_len)
{
    ...
    // msg data
    {
        ...
        do {
            ...
            packet->un.sendmsg.un.msg_iov.id = 0;
            packet->data                     = NULL;
    ...
    int             fd)         // Host OS descriptor or -1 if need new
{
    ...
    if (IPPROTO_TCP == protocol){
        ...
        sk = sk_alloc(net, PF_HOSTOS, GFP_KERNEL, &hostos_tcp_prot, false);
        if (sk == NULL){
            err = -ENOBUFS;
        ...
        sk = sk_alloc(net, PF_HOSTOS, GFP_KERNEL, &hostos_udp_prot, false);
        if (sk == NULL){
            err = -ENOBUFS;
