...
static int tun_net_xmit_ipnoise(struct sk_buff *skb, struct net_device *dev)
{
    int res;
...
int ipnoise_set_mac_address(struct net_device *dev, struct sockaddr *sa)
{
    int err = 0;
    strncpy(dev->dev_addr, sa->sa_data, sizeof(dev->dev_addr));
    return err;
}
...
ret:
    ...
    if (saddr){
        MDEBUG("build header, src: '%s'\n", saddr);
    }
    if (daddr){
        MDEBUG("build header, dst: '%s'\n", daddr);
    }
...
int ipnoise_hops_rebuild(struct sk_buff *skb)
{
    ...
    MDEBUG("rebuild header, hdr->h_source: '%s'\n", hdr->h_source);
...
static void tun_net_init(struct net_device *dev)
{
    ...
    case TUN_TUN_IPNOISE_DEV:
	    ...
	    dev->hard_start_xmit = tun_net_xmit_ipnoise;
		break;
	}
}
...
static int tun_set_iff(struct net *net, struct file *file, struct ifreq *ifr)
{
	...
	if (tun) {
		...
		if (ifr->ifr_flags & IFF_TUN) {
			...
			flags |= TUN_TUN_DEV;
			name = "tun%d";
		} else if (ifr->ifr_flags & IFF_TUN_IPNOISE) {
			/* TUN IPNoise device */
			flags |= TUN_TUN_IPNOISE_DEV;
			name = "ipnoise%d";
