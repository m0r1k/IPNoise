...
#include "nc_udp_client.h"

static void nc_udp_client_read_cb(
    struct bufferevent  *a_bev,
    void                *a_ctx)
{
    int32_t                         res, fd     = -1;
    struct  nc_udp_client_context   *context    = NULL;

    // get context
    context = (struct nc_udp_client_context *)a_ctx;
    if (!context){
    ...
    PDEBUG(10, "nc_udp_client_read_cb, fd: '%d'\n", fd);
...
static void nc_udp_client_write_cb(
    struct bufferevent  *a_bev,
    void                *a_ctx)
{
    int32_t fd = -1;
    ...
    PDEBUG(10, "nc_udp_client_write_cb, fd: '%d'\n", fd);
}

static void nc_udp_client_event_cb(
    struct bufferevent  *a_bev,
    short               a_what,
    void                *a_ctx)
{
    ...
    PDEBUG(10, "nc_udp_client_event_cb, fd: '%d',"
        " a_what: 0x'%x'\n",
        fd,
        a_what
    );

    PWARN("nc_udp_client_event_cb, fd: '%d',"
        " a_what: 0x'%x'\n",
        fd,
        a_what
    );
...
static int32_t nc_udp_client_stdin(
    struct event_base               *a_base,
    struct nc_udp_client_context    *a_context)
{
    int32_t     res, err    = -1;
    int32_t     fd          = FD_STDIN;
    int32_t     options     = 0;
    ...
    bufferevent_setcb(
        a_context->bev_stdin,
        nc_udp_client_read_cb,
        nc_udp_client_write_cb,
        nc_udp_client_event_cb,
        a_context
    );
...
static int32_t nc_udp_client_connect(
    struct event_base               *a_base,
    struct nc_udp_client_context    *a_context)
{
    int32_t     res, err    = -1;
    ...
    PINFO("sendto, was wrote: '%d' byte(s)\n", wrote);
...
int32_t nc_udp_client()
{
    int32_t                         res, err    = -1;
    struct  event_base              *base       = NULL;
    struct  nc_udp_client_context   *context    = NULL;

    int32_t nc_udp_client_context_len = 0;

    // get context size
    nc_udp_client_context_len = sizeof(*context);

    // alloc context info
    context = (struct nc_udp_client_context *)malloc(
        nc_udp_client_context_len
    );
    if (!context){
    ...
    memset(context, 0x00, nc_udp_client_context_len);
    ...
    // trying to connect
    res = nc_udp_client_connect(
        base,
        context
    );
    if (res){
        PERROR("nc_udp_client_connect failed\n");
        goto fail;
    }
