    ...
    int             blen)
{
    ...
    if (type == ARPHRD_IPNOISE){
        snprintf(buf, blen, "%s", addr);
        return buf;
    }
    if (alen == 4
        && (   type == ARPHRD_TUNNEL
...
{
    do {
        // IPNoise support (morik)
        //
        // check on "string" addresses
        // (looks like: 's:address_here')
        if (arg[0] != 's'){
            break;
        }
        if (arg[1] != ':'){
        ...
        arg += 2;
        // copy address
        strncpy(lladdr, arg, len);
        return strlen(lladdr);
    } while (0);
