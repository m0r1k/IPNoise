...
NeuronIPNoiseLink::NeuronIPNoiseLink()
    :   SUPER_CLASS()
{
    do_init();
};
...
void NeuronIPNoiseLink::do_init()
{
    m_sd                = -1;
    m_socket_notifier   = NULL;

    initType(PROP_NEURON_TYPE("core.neuron.ipnoise.link"));
    initIfIndex(PROP_INT(-1));
    initCheckTime(PROP_TIME());

    // register API
}

int32_t NeuronIPNoiseLink::do_autorun()
{
    int32_t err = 0;
    return err;
}

void NeuronIPNoiseLink::getAllProps(
    PropMapSptr a_props)
{
...
int32_t NeuronIPNoiseLink::parseBSON(
    mongo::BSONObj  a_obj)
{
    int32_t err         = 0;
    int32_t if_index    = 0;
...
int32_t NeuronIPNoiseLink::apiObjectUpdate(
    ...
    const PropMapSptr   a_props)
{
    ...
    if (res){
        PERROR("Cannot update props"
            " for object with ID: '%s'\n",
...
void NeuronIPNoiseLink::readSlot(
    int32_t a_fd)
{
...
//    vector <DomElement *>   users;
    ...
    skb = createNeigh<NeuronIPNoiseSkb>();
    if (!skb){
    ...
    // received packet
    skb->setInContent(PROP_STRING(ptr));
    skb->setInLinkName(PROP_STRING(ifname));
    skb->setInLinkIndex(PROP_INT(ifindex));
    skb->setFrom(PROP_STRING(in6addr2huid(&ipv6->ip6_src)));
    skb->setTo(PROP_STRING(in6addr2huid(&ipv6->ip6_dst)));
...
int32_t NeuronIPNoiseLink::setupLink(
    ...
    const int32_t   a_ifindex)
{
    ...
    setName(PROP_STRING(a_ifname));
    setIfIndex(PROP_INT(a_ifindex));
    setCheckTime(PropTime::now());
...
int32_t NeuronIPNoiseLink::getInterfacesInfo(
...
void NeuronIPNoiseLink::resetCheckTime()
{
    setCheckTime(PropTime::now());
}

ssize_t NeuronIPNoiseLink::send(
...
int32_t NeuronIPNoiseLink::init()
{
    int32_t err = 0;
    return err;
}

void NeuronIPNoiseLink::destroy()
{
}

NeuronSptr NeuronIPNoiseLink::object_create()
{
    NeuronSptr neuron(new NeuronIPNoiseLink);
    return neuron;
}

static ModuleInfo info = {
    .type           = "core.neuron.ipnoise.link",
    .init           = NeuronIPNoiseLink::init,
    .destroy        = NeuronIPNoiseLink::destroy,
    .object_create  = NeuronIPNoiseLink::object_create,
    .flags          = 0
};
