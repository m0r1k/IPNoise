...
<div class="neuron_user_map">
<perl>
    # 1*1 3*3 5*5 7*7 9*9
    my $zoom_level  = 7;
    my $ret         = '<table class="neuron_user_map_table">';
    ...
    for (my $row = 0; $row < $rows; $row++){
        ...
        for (my $col = 0; $col < $cols; $col++){
            ...
            {
                $td .= '<td'
                    .' class="neuron_user_map_center">'
                    .'<center>';
                $td .= $cur_neuron->renderEmbedded('icon');
                $td .= '</center></td>';
            } else {
                $td .= '<td'
                    .' class="neuron_user_map_item">'
                    .'<center>';
                my $neigh = $neighs_xy->{$row}->{$col};
                if ($neigh){
                    $td .= $neigh->renderEmbedded('icon');
                } else {
                    $td .= $cur_neuron->renderEmbedded(
                       'icon_map_add'
                    );
                }
                $td .= '</center></td>';
