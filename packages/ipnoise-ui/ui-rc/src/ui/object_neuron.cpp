...
UiObjectNeuron::UiObjectNeuron(
    UiViewer *a_viewer)
    :   UiObject(
            a_viewer,
            UiObject::UI_TYPE_NEURON
        )
{
    m_charge = 0;
}
...
float UiObjectNeuron::getCharge() const
{
    return m_charge;
}

void UiObjectNeuron::normalize()
{
    if (m_charge < -1){
        m_charge = -1;
    }
...
void UiObjectNeuron::inc()
{
    setCharge(m_charge + 0.05f);
    normalize();
}

void UiObjectNeuron::dec()
{
    setCharge(m_charge - 0.05f);
    normalize();
}

void UiObjectNeuron::draw()
{
...
UiObjectLineSptr UiObjectNeuron::getLink(
    UiObjectNeuron *a_neuron)
{
    LinksConstIt        it;
        ...
        it++)
    {
        ...
        if (    !cur_link->m_neuron_to
            ||  cur_link->m_neuron_to != a_neuron)
        {
            continue;
        }
        ret = cur_link;
...
UiObjectLineSptr UiObjectNeuron::getFreeLink()
{
    LinksConstIt        it;
    UiObjectLineSptr    ret;
    for (it = m_links.begin();
...
void UiObjectNeuron::processHits(
    GLint   hits,
    GLuint  buffer[])
{
    unsigned int    i, j;
    ...
    vector<UiObjectNeuronSptr>              neighs;
    vector<UiObjectNeuronSptr>::iterator    neighs_it;
    ...
    for (i = 0; i < (uint32_t)hits; i++){
        ...
        neuron = dynamic_pointer_cast<UiObjectNeuron>(object);
        if (!neuron){
            // skip non neuron
            continue;
        }
    ...
    for (neighs_it = neighs.begin();
        ...
        neighs_it++)
    {
        UiObjectNeuronSptr  neigh = *neighs_it;
...
void UiObjectNeuron::selectObjects()
{
    selectObjects(1, 0, 0);
    selectObjects(0, 1, 0);
    selectObjects(0, 0, 1);
...
void UiObjectNeuron::searchLinks()
{
    LinksConstIt it;
...
void UiObjectNeuron::getConnectedLinks(
    vector<UiObjectLineSptr>    &a_out,
    UiObjectNeuron              *a_skip_neuron)
{
    LinksConstIt it;
    ...
    {
        UiObjectLineSptr    link        = *it;
        UiObjectNeuron      *neuron_to  = NULL;

        neuron_to = link->m_neuron_to;
        if (!neuron_to){
            continue;
        }
        if (    a_skip_neuron
            &&  a_skip_neuron == neuron_to)
        {
            continue;
        }
    ...
    const float &a_charge)
{
    ...
    LinksIt             links_it;
    UiObjectNeuronSptr  neuron = viewer->m_object_neuron;
    UiObjectsNeuronsIt  it;

    if ((void *)neuron.get() == this){
        PFATAL("attempt to set charge for root neuron\n");
    }
    ...
    //);
    //float step      = 0; //len / (m_charge + neuron->m_charge + 0.000001f);
    //float charge    = 0;
    //for (it = viewer->m_objects_neurons.begin();
    //    it != viewer->m_objects_neurons.end();
    //    it++)
    //{
...
/*
    ...
    if (    0 == neuron->m_neuron->morik_row
        &&  0 == neuron->m_neuron->morik_col)
    {
    //    PWARN("MORIK len: '%f', m_charge: '%f'\n", len, m_charge);
    }
    if (    0  == neuron->m_neuron->morik_row
        &&  79 == neuron->m_neuron->morik_col)
    {
    //    PWARN("MORIK len: '%f', m_charge: '%f'\n", len, m_charge);
    }
    if (    0  == neuron->m_neuron->morik_row
        &&  39 == neuron->m_neuron->morik_col)
    {
    //    PWARN("MORIK len: '%f', m_charge: '%f'\n", len, m_charge);
    }
    if (    29 == neuron->m_neuron->morik_row
        &&  39 == neuron->m_neuron->morik_col)
    {
    //    PWARN("MORIK len: '%f', m_charge: '%f'\n", len, m_charge);
    }
*/
    ...
    if (TAIL_PERIOD && ++tail >= TAIL_PERIOD){
        tail = 0;
        UiObjectNeuronSptr tail;
        tail = UiObjectNeuronSptr(new UiObjectNeuron(
            getViewer()
        ));
    ...
    for (links_it = neuron->m_links.begin();
        links_it != neuron->m_links.end();
        links_it++)
    {
...
void UiObjectNeuron::selected()
{
    PWARN("object neuron selected\n");
...
/*
    if (m_neuron){
        PWARN("selected neuron row: '%d', col: '%d',"
            " ax: '%f', ay: '%f', az: '%f'\n",
            m_neuron->morik_row,
            m_neuron->morik_col,
            ax, ay, az
...
*/
    ...
    if (0 && m_neuron){
        QMessageBox::information(
            0, "Selection",
            QString::fromUtf8(m_neuron->serialize().c_str())
        );
    }
}
