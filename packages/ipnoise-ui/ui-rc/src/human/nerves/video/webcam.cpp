...
/*
    if (0){
        ...
        float               xx, yy, zz;
        neuron = getViewer()->m_objects_neurons[0];
        neuron->m_frame.getPosition(xx, yy, zz);
        m_viewer->m_object_neuron->m_frame.setPosition(
            xx, yy, zz
        );
    }
...
//                       morik = 1;
//                       float xx, yy, zz;
//                       neuron = getViewer()->m_objects_neurons[index];
//                       neuron->m_frame.getPosition(xx, yy, zz);
//                       neuron->m_charge = charge;
//                       m_viewer->m_object_neuron->m_frame.setPosition(
//                           xx, yy, zz
//                       );
//                       continue;
...
/*
    if (bright_delta > 0){
        float               ax, ay, az;
        UiObjectNeuronSptr  memory;
        getViewer()->m_object_neuron->m_frame.getPosition(
            ax, ay, az
        );
        memory = UiObjectNeuronSptr(new UiObjectNeuron(
            getViewer()
        ));
...
/*
    ...
    if (0){
        ...
        neuron = getViewer()->m_object_neuron;
        neuron->m_frame.getPosition(
            ax, ay, az
        );

        for (it = getViewer()->m_objects_neurons.begin();
            it != getViewer()->m_objects_neurons.end();
            it++)
        {
            float               bx, by, bz;
            UiObjectNeuronSptr  cur_neuron = it->second;

            row = cur_neuron->m_neuron->morik_row;
            col = cur_neuron->m_neuron->morik_col;

            cur_neuron->m_frame.getPosition(
                bx, by, bz
            );
            ...
            NeuronSptr  neuron = cur_neuron->m_neuron;
            int32_t     preview_y = 220 + neuron->morik_row*3;
            int32_t     preview_x = 20  + neuron->morik_col*3;
