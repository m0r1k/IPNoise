...
{
// MORIK {
//    printf("[MORIK NET TOLS: %s %s:%d] name: '%s'\n", __FILE__, __FUNCTION__, __LINE__, name);
// MORIK }     
...
static int if_readconf(void)
{
    ...
    for (n = 0; n < ifc.ifc_len; n += sizeof(struct ifreq)) {
        if (ifr->ifr_name != NULL && strlen(ifr->ifr_name)){
...
int if_fetch(struct interface *ife)
{
    ...
    if (ioctl(skfd, SIOCGIFHWADDR, &ifr) < 0)
        memset(ife->hwaddr, 0, MAX_ADDR_LEN);
    else
    	memcpy(ife->hwaddr, ifr.ifr_hwaddr.sa_data, MAX_ADDR_LEN);
